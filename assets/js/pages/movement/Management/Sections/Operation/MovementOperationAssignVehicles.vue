        ...
        <div class="kt-portlet__body">
            ...
            <div class="kt-datatable kt-datatable--default kt-datatable--brand kt-datatable--loaded">
                <erp-ajax-table
                    ...
                    "
                    :parameters-default="getDefaultParams"
                    :columns="columns"
                    :options="options"
                    :firstCall="false"
                />
...
import AssignVehiclesFilter from "./MovementOperationAssignVehiclesFilter.vue";
import MovementAssignVehiclesCart from "../../../components/MovementAssignVehiclesCart.vue";

export default {
    name: "MovementOperationAssignVehicles",
    components: {
        ErpAjaxTable,
    ...
    computed: {
        getDefaultParams() {
            if (this.movement.vehicleFilter.length > 0) {
                return {
                    "vehicleTypeId[]": this.movement.vehicleFilter?.vehicleType.map((item) => item.id),
                    "saleMethodId[]": this.movement.vehicleFilter?.saleMethod.map((item) => item.id),
                };
            }
    ...
    methods: {
        back() {
            location.href = this.routing.generate("movement.vehicleLine.assigned.list", {
                id: this.movement.id,
            });
        },
        downloadExcel() {

<template>
    <fragment>
        <div class="kt-portlet kt-portlet--bordered">
            <div class="kt-portlet__body">
                ...
                <div class="kt-datatable kt-datatable--default kt-datatable--brand kt-datatable--loaded">
                    <erp-ajax-table
                        ref="movementCarrierTable"
                        id="movementCarrierTable"
        ...
        <modal-delivery-notes-movement :row="row" />
...
import ModalCancelMovement from "../../../components/modals/ModalCancelMovement.vue";
import ModalDeliveryNotesMovement from "../../../components/modals/ModalDeliveryNotesMovement.vue";
import ModalValidationMovement from "../../../components/modals/ModalValidationMovement.vue";

export default {
    name: "ListMovementCarrier",
    components: {
        ErpAjaxTable,
        ModalCancelMovement,
        ModalDeliveryNotesMovement,
    ...
    data() {
        return {
            ...
            columns: [
                ...
                {
                    field: "",
                    title: txtTrans.fields.amount,
                    sortable: true,
                    formatter: (value, row) => Formatter.formatField(row.manualCost ?? row.automaticCost),
                },
                {
                ...
                },
                {
                    ...
                    formatter: this.actionsFormatterCarrier,
                    events: {
                        ...
                        "click .file": (e, value, row) => this.clickDeliveryNotesRow(row),
    ...
    methods: {
        ...
        clickDeliveryNotesRow(row) {
            this.row = row;
            $("#modal-delivery-notes").modal("show");
        },
        clickCancelRow(row) {
        ...
        formatterAssignedLicensePlateUnits(value, row) {
            return `
                ${this.txt.form.licensePlateLoad}: ${Formatter.formatField(
                row.assignedLicensePlateUnits.load
            )}/${Formatter.formatField(row.assignedLicensePlateUnits.total)}
                \n
                ${this.txt.form.licensePlateUnload}: ${Formatter.formatField(
                row.assignedLicensePlateUnits.unload
            )}/${Formatter.formatField(row.assignedLicensePlateUnits.total)}
            `;
        },
        ...
        formatterVehicleActualLoadDate(value, row) {
            ...
            const allSame = Object.values(dateCounts).some((count) => count === row.vehicle.length);
            if (allSame || filteredDates.length >= 1) {
        ...
        actionsFormatterCarrier(value, row) {
            const actions = [
            ...
            // Solo se pueden cancelar por usuario de creación
            let isUserCreator = row.creationUser.id == this.loginUser.id;
            if (row.movementStatus.id === constants.movementStatus.pending && isUserCreator) {
                actions.push(
            ...
            if (
                ...
                if (row.vehicle.length > 0) {
                    actions.push(
                        `<a href="javascript:void(0)" class="btn btn-sm btn-clean btn-icon btn-icon-md file" title="${txtTrans.form.deliveryNotes}"><i class="flaticon-file"></i></a>`
                    );
                }
            }
        ...
        exportCSV() {
            ...
            this.axios
                ...
                .then((response) => {
                    ...
                    link.href = url;
                    link.setAttribute("download", "movement_carrier_list.csv");
        ...
        vehicleExportCSV() {
            ...
            this.axios
                ...
                .then((response) => {
                    ...
                    const link = document.createElement("a");
                    link.href = url;
                    link.setAttribute("download", "movement_carrier_vehicle_list.csv");
                    document.body.appendChild(link);
                    link.click();
                })
                .catch(async (error) => {
...
<style>
#movementCarrierTable tbody tr:has(td.cancelled) {
    background-color: #d5d5d5;
}
#movementCarrierTable tbody tr:has(td.cancelled):hover {
    background-color: #d5d5d5 !important;
}
#movementCarrierTable tbody tr:hover {
